...
For cross-tool orchestration, run `tsx scripts/furry-reference-bridge.ts` and write the programmatic metadata manifest to its standard input; it emits only local paths and a temporary directory. Invoke `image_gen` with those paths, then run the same helper with a cleanup request on standard input. Do not put capabilities in a command argument, tool text, or log. This route is for Codex-local tool orchestration only; browser and mobile hosts still need native attachment plumbing.
