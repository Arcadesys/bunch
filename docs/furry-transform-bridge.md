...
The current Codex host exposes a secure widget-media path but no supported inverse API that turns Bunch metadata into assistant image-editor attachments. The bridge therefore records and prepares the secure handoff, with runtime image attachment blocked on that host capability.
