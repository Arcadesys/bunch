...
ChatGPT must read available messages, report topics, decisions, open matters and coverage gaps, and show the window and duration. DIDdy does not automatically receive ChatGPT history. The host drafts the brief first; only after the user reviews it and explicitly authorizes the write may it save the synthesis with its window and coverage gaps for 30 days using `save_conversation_catch_up`. Raw transcripts remain prohibited. This is a ChatGPT MCP follow-up; a website mutation cannot independently start a ChatGPT conversation. Runtime history retrieval and generation depend on the connected host's capabilities.

For a new fronting entry, the host must search the authenticated owner-scoped profiles and proceed only with one exact active name match. It records the explicitly confirmed episode, then uses that exact profile and period to select the most recent prior recorded `FRONTING` end. That bounded interval is a `RECORDED_FRONTING_WINDOW`, never evidence of absence, replacement, or current fronting. A configured low-cost host model (for example, a Luna-class summarizer) may compose the brief, but DIDdy never selects, purchases, or calls an external model API. The host must identify its source client, disclose its available conversation sources and gaps, retain no raw transcripts, and stop at `NEEDS_DATES` or `HOST_REQUIRED` rather than fabricating coverage.
...
The Switch dock is pinned to the bottom of every page and always shows the recorded host and open fronting episodes. Opening it (the button or `S`) opens a modal with every active profile shown as a face. The modal keeps keyboard focus inside it, closes with its Close button or Escape, and scrolls independently on short or enlarged-text viewports. "Record a switch as" decides what a tap writes: **Host** replaces the open hosting period through the existing host setter, and **Also here** starts a fronting episode. Tapping someone who already carries that role ends it instead of writing a duplicate. The tap is the explicit report and uses the same idempotent endpoints as the Home form, which remains available with its separate confirmation step.
