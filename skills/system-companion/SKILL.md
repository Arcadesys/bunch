...
- For an explicit image request with an uploaded scene, object, or style image plus named alters, call `prepare_chatgpt_alter_image`. The uploaded image is image 1; ordered private appearance-reference images follow it. The widget transfers reference bytes to transient ChatGPT files with `library: false`, then keeps capabilities widget-only and asks ChatGPT's image tool to generate.
- For a named-alter request without an uploaded scene/object/style image, call `generate_scene` directly with exact names (see Native private images). Do not call a prepare tool first: those packets are for external image-studio adapters, except the explicit ChatGPT handoff above.
...
- Use `generate_scene` when the user explicitly asks for a new image without an uploaded scene/object/style image, such as “draw Lucy in a cozy sweater.” Bunch attaches every selected appearance reference on its server, so this is the native path for drawing named alters in chat. Never ask the user to upload or attach a photo Bunch already holds. It accepts a scene, optional exact active alter names, format, and a request ID. The result is a job, not an image until its state is `COMPLETE`.
- Use `prepare_chatgpt_alter_image` when an explicit image request includes an uploaded scene/object/style image and named alters. The scene image is image 1; the widget uploads private references as transient files with `library: false`, keeps capabilities widget-only, and sends one image-generation handoff. If the file, fetch, upload, or handoff fails, stop without claiming generation. There is no prose-only likeness fallback.
