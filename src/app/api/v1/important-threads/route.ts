...
export async function GET(request: Request) {
  return apiResponse(async () => {
    ...
    return NextResponse.json({ data: await getCatchUpService().listThreads(ownerId), meta: {} });
  });
}
...
export async function POST(request: Request) {
  return apiResponse(async () => {
    ...
    const result = await getCatchUpService().suggestThread(ownerId, { ...(await jsonBody(request)), requestId } as never, "WEB");
