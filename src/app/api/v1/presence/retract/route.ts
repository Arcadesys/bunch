...
import { retractPresenceChangeSchema } from "@/domain/presence";
...
export async function POST(request: Request) {
  return apiResponse(async () => {
    ...
    const input = retractPresenceChangeSchema.parse({ ...(await jsonBody(request)), requestId });
    const result = await getSystemService().retractPresenceChange(owner, input, "WEB");
