...
function LauncherRow({ href, icon, title, detail, primary = false }: { href: string; icon: Parameters<typeof Icon>[0]["name"]; title: string; detail: string; primary?: boolean }) {
  return <Link className={`launcher-row${primary ? " primary" : ""}`} href={href}><Icon name={icon}/><span><strong>{title}</strong><small>{detail}</small></span><span className="launcher-arrow" aria-hidden="true">›</span></Link>;
}
...
export function HomeLauncher() {
  ...
  return <main className="app-home"><AppNavigation current="CATCH_UP"/><div className="launcher-layout">
    <section className="launcher-main" aria-labelledby="launcher-heading"><header><h1 id="launcher-heading">What would help right now?</h1><p>Choose one place to begin.</p></header>
      <nav className="launcher-actions" aria-label="Things you can do">
      ...
      <details className="launcher-more"><summary><Icon name="more"/><span><strong>More places</strong><small>Save a thread, history, and options.</small></span></summary><nav aria-label="More places"><Link href="/threads#create-record">Save a thread</Link><Link href="/history">History</Link><Link href="/options">Options</Link></nav></details>
    </section>
    <aside className="right-now" aria-labelledby="right-now-heading"><div className="right-now-heading"><h2 id="right-now-heading">Right now</h2>{session ? <p><strong>{session.reviewedCount} of {session.totalCount}</strong> reviewed</p> : null}</div>
      {session ? <div className="launcher-progress" aria-label={`${session.reviewedCount} of ${session.totalCount} reviewed`}><span style={{ width: `${session.totalCount ? session.reviewedCount / session.totalCount * 100 : 100}%` }}/></div> : null}
      {state === "loading" ? <p role="status">Reading your catch-up…</p> : state === "unauthorized" ? <><p>Sign in to read private records.</p><a className="command-button" href="/auth/login">Sign in with Google</a></> : state === "error" ? <p role="alert">Catch-up could not be read. Your saved tools are still available.</p> : <section className="next-step"><h3>Next step</h3><p><strong>{next?.title ?? "Choose one saved task"}</strong></p><p>{next?.nextAction ?? "Open Todos when you’re ready."}</p><Link className="command-button" href={recordHref}>Open todo <span aria-hidden="true">›</span></Link></section>}
    </aside>
  </div></main>;
}
