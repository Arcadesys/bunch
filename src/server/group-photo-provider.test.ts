...
import { openAIGroupPhotoProvider, openAINativeSceneProvider, photoFinisherAvailable } from "./group-photo-provider";
...
test("native scenes use generations without references and edits with every reference", async () => {
  ...
  try {
    ...
    await openAINativeSceneProvider({ prompt: "A quiet room", model: "test", size: "1024x1024", references: [] });
    await openAINativeSceneProvider({ prompt: "Two people", model: "test", size: "1024x1024", references: [{ bytes: new Uint8Array([1]), contentType: "image/png", name: "first" }, { bytes: new Uint8Array([2]), contentType: "image/jpeg", name: "second" }] });
    assert.equal(calls[0].url, "https://api.openai.com/v1/images/generations");
    assert.deepEqual(JSON.parse(String(calls[0].options?.body)), { model: "test", prompt: "A quiet room", size: "1024x1024", quality: "high", output_format: "jpeg" });
