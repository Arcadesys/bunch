...
test("missing text needs information unless a selected reference exists; gallery alone is insufficient", () => {
  ...
  const ready = buildAlterImagePrompt("Draw everyone", [twi, { ...missing, appearanceReference: picture }]);
...
test("all follows every page and associates reference capabilities with the correct person only in metadata", async () => {
  ...
  try {
  const profiles = [twi, { ...twi, id: randomUUID(), name: "Second" }].map((profile) => ({ ...profile, appearanceReference: { id: randomUUID(), contentType: "image/png" as const, isProfilePicture: false, createdAt: twi.createdAt } }));
    const calls: unknown[] = [];
    const service = {
    ...
    assert.deepEqual(result._meta.referenceMedia.map((media) => [media.alterId, media.imageId]), profiles.map((profile) => [profile.id, profile.appearanceReference!.id]));
