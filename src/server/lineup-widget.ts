...
<body>
<main class="shell">
  <header class="top"><span class="brand">Bunch</span><a class="link" id="open-site" href="${publicOrigin}/profiles" target="_blank" rel="noopener noreferrer">Manage profiles</a></header>
...
<script>
const SITE=${safeOrigin};const byId=id=>document.getElementById(id);let imageManifest=[];let latestState=null;let initialized=false;
...
function showError(){byId('notice').textContent='The private lineup could not load. Ask to open the lineup again, or use Manage profiles above.';}
const loadingTimer=setTimeout(()=>{if(!latestState)showError();},15000);
function receiveResult(result){if(result?.isError){clearTimeout(loadingTimer);showError();return;}if(!Array.isArray(stateFrom(result).profiles)){clearTimeout(loadingTimer);showError();return;}render(result);}
window.addEventListener('message',event=>{
  if(event.source!==window.parent)return;
  const message=event.data;
  if(!message||message.jsonrpc!=='2.0')return;
  if(message.id==='lineup-initialize'&&!initialized){
    if(message.error){if(!latestState)showError();return;}
    if(message.result){initialized=true;window.parent.postMessage({jsonrpc:'2.0',method:'ui/notifications/initialized',params:{}},'*');}
    return;
  }
  if(message.method==='ui/notifications/tool-result')receiveResult(message.params);
  if(message.method==='ui/notifications/tool-cancelled'){clearTimeout(loadingTimer);showError();}
},{passive:true});
window.addEventListener('openai:set_globals',event=>{
  const globals=event.detail?.globals;
  if(!globals)return;
  const value=globals.toolOutput??latestState??window.openai?.toolOutput;
...
if(window.parent!==window)window.parent.postMessage({jsonrpc:'2.0',id:'lineup-initialize',method:'ui/initialize',params:{protocolVersion:'2026-01-26',appInfo:{name:'Bunch lineup',version:'3.0.0'},appCapabilities:{availableDisplayModes:['inline']}}},'*');
</script>
</body>
</html>`;
}
