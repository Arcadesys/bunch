...
import { defaultStickerSlots, saveStickerPackSchema, stickerPackViewSchema, stickerSlotSchema } from "@/domain/sticker-pack";
import { getNativeSceneService, type NativeSceneService } from "@/server/native-scene-service";
import { getStickerPackService, type StickerPackService } from "@/server/sticker-pack-service";
...
export function createMcpServer(ownerId: string, serviceOverride?: ReturnType<typeof getSystemService>, catchUpOverride?: ReturnType<typeof getCatchUpService>, profileRepository: Pick<SystemRepository, "listProfiles"> = repository, summaryOverride?: ConversationSummaryService, scheduleNativeScene?: (ownerId: string, renderId: string) => void, nativeSceneServiceOverride?: Pick<NativeSceneService, "start" | "get" | "list"> & Partial<Pick<NativeSceneService, "allowance">>, loadAccount?: (ownerId: string) => Promise<{ display_name: string } | null>, stickerPackServiceOverride?: Pick<StickerPackService, "get" | "list" | "save">) {
  ...
  const stickerPacks = () => stickerPackServiceOverride ?? getStickerPackService();
  server.registerTool("get_sticker_pack", {
    title: "Get personalized sticker pack",
    description: "Read the saved ten-reaction creative board for one explicitly selected alter. Returns the default semantic slots when no board exists. These slots are intents, not fixed gestures.",
    inputSchema: { alterId: uuidSchema },
    outputSchema: { data: stickerPackViewSchema.nullable(), defaults: z.array(stickerSlotSchema) },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, async ({ alterId }) => {
    const data = await stickerPacks().get(ownerId, alterId);
    return {
      structuredContent: { data, defaults: defaultStickerSlots() },
      content: [{ type: "text", text: data ? `Loaded the saved sticker board for ${data.alterName}. Treat its ten slots as approved or draft acting direction according to its status.` : "No sticker board is saved yet. Start from the ten semantic defaults and interview for how this person actually expresses them." }],
    };
  });
  server.registerTool("list_sticker_packs", {
    title: "List personalized sticker packs",
    description: "List saved personalized reaction sticker boards for active alters.",
    inputSchema: {},
    outputSchema: { data: z.array(stickerPackViewSchema) },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, async () => {
    const data = await stickerPacks().list(ownerId);
    return { structuredContent: { data }, content: [{ type: "text", text: `Listed ${data.length} saved sticker pack${data.length === 1 ? "" : "s"}.` }] };
  });
  server.registerTool("save_sticker_pack", {
    title: "Save personalized sticker pack",
    description: "Save the ten-reaction creative contract for one explicitly selected alter after the user approves it, or record the final Telegram add-pack URL after successful publication. Never infer the alter from presence.",
    inputSchema: { alterId: uuidSchema, ...saveStickerPackSchema.shape },
    outputSchema: { data: stickerPackViewSchema, meta: responseMetaSchema },
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false },
  }, async ({ alterId, ...input }) => {
    const result = await stickerPacks().save(ownerId, alterId, input, "MCP");
    return {
      structuredContent: { data: result.data, meta: { requestId: input.requestId, replayed: result.replayed } },
      content: [{ type: "text", text: result.data.status === "PUBLISHED" ? "Saved the published Telegram sticker-pack link with this person." : result.data.status === "APPROVED" ? "Saved the user-approved ten-reaction sticker board." : "Saved the sticker-board draft." }],
    };
  });
