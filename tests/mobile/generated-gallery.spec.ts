...
test("gallery shows photo tiles, preserves them on older-page failure, retries and reloads", async ({ page }, testInfo) => {
  ...
  // One control per tile: the tile itself. Actions live in the detail pane.
  await expect(page.getByRole("link", { name: "Download" })).toHaveCount(0);
  await page.getByRole("button", { name: "Load older photos" }).click();
  await expect(page.getByRole("region", { name: "Gallery" }).getByRole("alert")).toContainText("Could not load");
  await expect(tile).toBeVisible();
  failOlder = false;
...
test("the detail pane shows the full photo with its actions", async ({ page }) => {
  ...
  await tile.click();
  const detail = page.getByRole("region", { name: "Gallery" });
  await expect(detail.getByRole("img", { name: long })).toBeVisible();
  await expect(detail.getByText(long, { exact: true })).toBeVisible();
  await expect(detail.getByRole("link", { name: "Download" })).toHaveAttribute("href", "/api/v1/native-scenes/renders/first/image");
  await expect(detail.getByRole("link", { name: "Reopen scene" })).toHaveAttribute("href", "/images?render=first");
  // Click the next tile
  const nextTile = page.getByRole("button", { name: /^Group photo/ });
  await nextTile.click();
  await expect(detail.getByRole("link", { name: "Reopen group photo" })).toBeVisible();
  // Go back to first tile
  const firstTile = page.getByRole("button", { name: /Lucy and Tally/ });
  await firstTile.click();
  await expect(detail.getByRole("link", { name: "Reopen scene" })).toBeVisible();
});
...
test("gallery deletes a photo only after confirmation and explains a refusal", async ({ page }) => {
  ...
  const detail = page.getByRole("region", { name: "Gallery" });
  const gone = detail.getByRole("button", { name: "Delete" });
  await gone.click();
  ...
  await expect(page.locator('[role="status"]')).toContainText("The image was permanently deleted");
