...
for (const enlarged of [false, true]) {
  test(`home → note → save feedback → saved note → home${enlarged ? " at 200% text" : ""}`, async ({ page, harness }) => {
    ...
    // Saving closes the form and opens the saved note, with the feedback beside it.
    const feedback = page.locator(".command-notice");
    ...
    await (await openSections(page)).getByRole("link", { name: "Home", exact: true }).click();
...
test("home → todos → add → complete → home", async ({ page, harness }) => {
  ...
  await page.getByRole("button", { name: "+ Add a todo", exact: true }).click();
  ...
  // Saving closes the form and opens the saved todo, with the feedback beside it.
  await expect(page.locator(".command-notice")).toContainText("Todo saved");
...
test("home → save thread → confirm → home → people → home", async ({ page, harness }) => {
  ...
  // Split view: on phones the list shows first and a person's row opens their detail.
  const robinRow = page.locator(".ld-row").filter({ hasText: "Test Robin" });
  await expect(robinRow).toBeAttached();
  if (!(await page.locator(".ld-detail").isVisible())) await robinRow.click();
